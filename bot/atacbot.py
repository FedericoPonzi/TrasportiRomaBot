...
class AtacBot(object):
    ...
    def search_palina_from_location(self, location={'latitude': "41.90", 'longitude': "12.47"}):
        try:
            res = self.servers['paline'].paline.SmartSearch(self.token,
                                                            "punto:(" + str(location['latitude']) + " , " + str(
                                                                location['longitude']) + ")")
        except Fault as e:
    ...
    def get_orari_bus(self, id_percorso):
        ...
        try:
            res = self.servers['paline'].paline.Percorso(self.token, str(id_percorso), "",
                                                         datetime.now().strftime("%Y-%m-%d"), "it")
        except Fault as e:
            if e.faultCode == self.server_resp_codes['expired_session']:
                self.__update_token()
                return self.get_percorso_info(id_percorso)
            else:  # TODO: unknown_percorso
                ...
                if len(i['minuti']) > 0:
                    ...
                    for minuti in i['minuti']:
                        ...
                        if ind < len(lista_orari):
                            orario += ", "
                    orario += "\n"
            m = "Queste sono le partenze da ''" + capolinea + "' per oggi " + datetime.today().strftime(
                "%A, %d %B") + ":\n"
            m += orario
    ...
    def get_prossima_partenza(self, id_percorso):
        ...
        try:
            ...
            if diff_delta.days < 0:  # If this is negative, this means that data is more recent than datetime.now().
                return BotResponse(True, "Un autobus è partito da poco! :smile:")

            m = "La prossima partenza dal capolinea è alle " + str(data.strftime(frmtstr)) + " :blush:"
            m += ", ovvero fra " + utils.time_left(diff_delta)

            ret = BotResponse(True, m)
        except Fault as e:
            if e.faultCode == self.server_resp_codes['expired_session']:
                self.__update_token()
                return self.get_prossima_partenza(id_percorso)
            elif e.faultCode == self.server_resp_codes['unknown_percorso']:  # should never happen
                logger.error("Errore get_autobus_from_fermata richiesta palina ", id_percorso, ", errore:", e)
    ...
    def get_percorso_info(self, id_percorso):
        ...
        try:
            ...
            if e.faultCode == self.server_resp_codes['expired_session']:
                ...
                return self.generic_error
        res = res['risposta']
        m = "Informazioni per la linea " + res['percorso']['id_linea'] + " direzione " + res['percorso'][
            'arrivo'] + "\n\n"
        ...
        if not presente_veicolo:
            m += "\n Non ho informazioni riguardanti le posizioni dei veicoli sulla linea :worried: " \
                 "prova ad aggiornare!\n"
    ...
    def get_percorso(self, fr, to):
        ...
        try:
            ...
            if e.faultCode == self.server_resp_codes['expired_session']:
                self.__update_token()
                m = BotResponse(False, self.get_percorso(fr, to))
            else:
                m = self.generic_error
        return m

    def get_linee_from_palina(self, id_palina):
        # Questo metodo restituisce l'elenco delle linee che transitano per la palina id_palina,
        # con le relative informazioni (monitorata, abilitata)
        return self.servers['paline'].paline.PalinaLinee(self.token, id_palina)
